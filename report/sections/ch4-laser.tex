...
The full code written for the laser sensor can be found in Appendix \ref{code:laser-full}. The sequence diagram below gives us an overview of the calls happening in the code:
...
The initialization of ROS happens afterwards, by calling the \textit{init} function with arguments and the name of our module. The module name is used as the node name in ROS. In the next line, ROS also starts to advertise messages to the \textbackslash\textit{scan} topic, where all the scan results are being published to.
